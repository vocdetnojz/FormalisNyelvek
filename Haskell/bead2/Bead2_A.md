...
blocks](#metadata-blocks), [`Div` blocks](#extension-fenced_divs),
[footnotes](#footnotes) and [citations](#citations), embedded
[LaTeX](#extension-raw_tex) (including [math](#math)), [Markdown inside
HTML block elements](#extension-markdown_in_html_blocks), and much more.
These enhancements, described further under [Pandoc's
Markdown](#pandocs-markdown), can be disabled using the
`markdown_strict` format.
...
PDF output can be controlled using [variables for
LaTeX](#variables-for-latex) (if LaTeX is used) and [variables for
ConTeXt](#variables-for-context) (if ConTeXt is used). When using an
...
for [typography](#typography) if added to the template or included in
any header file. The [`natbib`](https://ctan.org/pkg/natbib),
[`biblatex`](https://ctan.org/pkg/biblatex),
[`bibtex`](https://ctan.org/pkg/bibtex), and
[`biber`](https://ctan.org/pkg/biber) packages can optionally be used
for [citation rendering](#citation-rendering).
...
:   Specify input format. *FORMAT* can be `native` (native Haskell),
    ...
    `twiki` (TWiki markup), `tikiwiki` (TikiWiki markup), `creole`
    (Creole 1.0), `haddock` (Haddock markup), or `latex` (LaTeX).
    (`markdown_github` provides deprecated and less accurate support for
    Github-Flavored Markdown; please use `gfm` instead, unless you need
    to use extensions other than `smart`.) Extensions can be
    individually enabled or disabled by appending `+EXTENSION` or
    `-EXTENSION` to the format name. See [Extensions](#extensions)
...
:   Specify output format. *FORMAT* can be `native` (native Haskell),
    ...
    `html4` (XHTML 1.0 Transitional), `html` or `html5` (HTML5/XHTML
    [polyglot markup](https://www.w3.org/TR/html-polyglot/)), `latex`
    ...
    document), `docx` (Word docx), `haddock` (Haddock markup), `rtf`
    (rich text format), `epub2` (EPUB v2 book), `epub` or `epub3` (EPUB
    v3), `fb2` (FictionBook2 e-book), `asciidoc` (AsciiDoc), `icml`
    ...
    and JavaScript slide show), `pptx` (PowerPoint slide show) or the
    ...
    and their names. See `--list-output-formats` and
...
:   Use *FILE* as a custom template for the generated document. Implies
    `--standalone`. See [Templates](#templates), below, for a
...
:   Include an automatically generated table of contents (or, in the
    ...
    option has no effect on `man`, `docbook4`, `docbook5`, or `jats`
    output.
...
:   Use the specified file as a style reference in producing a docx or
    ...
    :   For best results, the reference docx should be a modified
        ...
        pandoc: \[paragraph\] Normal, Body Text, First Paragraph,
        Compact, Title, Subtitle, Author, Date, Abstract, Bibliography,
        Heading 1, Heading 2, Heading 3, Heading 4, Heading 5, Heading
        6, Heading 7, Heading 8, Heading 9, Block Text, Footnote Text,
...
:   Use [KaTeX](https://github.com/Khan/KaTeX) to display embedded TeX
    ...
    library. If a *URL* is not provided, a link to the KaTeX CDN will be
    inserted.
...
:   *Deprecated.* Use the
    [LaTeXMathML](http://math.etsu.edu/LaTeXMathML/) script to display
    embedded TeX math in HTML output. TeX math will be displayed between
    `$` or `$$` characters and put in `<span>` tags with class `LaTeX`.
    The LaTeXMathML JavaScript will then change it to MathML. Note that
    currently only Firefox and Safari (and select e-book readers)
    natively support MathML. To insert a link the `LaTeXMathML.js`
    script, provide a *URL*.

`--jsmath`\[`=`*URL*\]

:   *Deprecated.* Use [jsMath](http://www.math.union.edu/~dpvc/jsmath/)
    (the predecessor of MathJax) to display embedded TeX math in HTML
    output. TeX math will be put inside `<span>` tags (for inline math)
    or `<div>` tags (for display math) with class `math` and rendered by
    the jsMath script. The *URL* should point to the script (e.g.
...
:   *Deprecated.* Enclose TeX math in `<eq>` tags in HTML output. The
    ...
    [gladTeX](http://ans.hsh.no/home/mgg/gladtex/) to produce images of
...
:   *Deprecated.* Render TeX math using the
    [mimeTeX](http://www.forkosh.com/mimetex.html) CGI script, which
    generates an image for each TeX formula. This should work in all
    browsers. If *URL* is not specified, it is assumed that the script
    is at `/cgi-bin/mimetex.cgi`.
...
blocks](#extension-yaml_metadata_block). They may also be set at the
command line using the `-V/--variable` option: variables set in this way
...
:   identifies the main language of the document, using a code according
    ...
    BCP 47) can be used to switch the language in that range. In LaTeX
...
Variables are available for [producing slide shows with
pandoc](#producing-slide-shows-with-pandoc), including all [reveal.js
configuration
...
:   color for internal links, citation links, external links, and links
    in table of contents: uses options allowed by
...
:   adds to the preamble the setup necessary to generate PDF/A-1b:2005.
    ...
    Profiles](http://wiki.contextgarden.net/PDFX#ICC_profiles). See also
    [ConTeXt PDFA](http://wiki.contextgarden.net/PDF/A) for more
    details.
...
used for highlighting, which works in HTML, Docx, Ms, and LaTeX/PDF
