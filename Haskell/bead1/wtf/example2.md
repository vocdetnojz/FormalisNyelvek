...
output through \[`iconv`\]:
    ...
    6   iconv -t utf-8 input.txt | pandoc | iconv -f utf-8
...
file.
    ...
    79   
    80       You can also modify the default `reference.pptx`: first run
...
(see `--css`):
    ...
    41       }
    42       body { font-family: "DejaVuSans"; }
...
`dir` : the base direction of the document, either `rtl` (right-to-left)
...
using this syntax:
    ...
    57   ~~~~ {#mycode .haskell .numberLines startFrom="100"}
    58   qsort []     = []
...
formatted as Markdown. This is useful for verse and addresses:
    ...
    61   | The limerick packs laughs anatomical
...
FictionBook2 ~ If the `--webtex` option is used, formulas are rendered
...
title, in quotes.)
    ...
    136   This is an [inline link](/url), and here's [one with
    137   a title](http://fsf.org "click here for a good time!").
...
prefixed with `mailto`:
    ...
    137   [Write me!](mailto:sam@green.eggs.ham)
...
Here are some examples:
    ...
    138   [my label 1]: /foo/bar.html  "My title, optional"
    139   [my label 2]: /foo
    140   [my label 3]: http://fsf.org (The free software foundation)
    141   [my label 4]: /bar#special  'A title in single quotes'

The URL may optionally be surrounded by angle brackets:

    139   [my label 5]: <http://foo.bar.baz>

The title may go on the next line:

    140   [my label 3]: http://fsf.org
    141     "The free software foundation"

Note that link labels are not case sensitive. So, this will work:

    141   Here is [my link][FOO]
    142   
    143   [Foo]: /bar/baz
...
In an *implicit* reference link, the second pair of brackets is empty:
    ...
    142   See [my website][].
    143   
    144   [my website]: http://foo.bar.baz
...
omitted entirely:
    ...
    144   See [my website].
    145   
    146   [my website]: http://foo.bar.baz
...
generated identifier (see \[Header identifiers\]). For example:
    ...
    145   See the [Introduction](#introduction).
...
link text will be used as the image's alt text:
    ...
    147   ![la lune](lalune.jpg "Voyage to the moon")
    148   
    149   ![movie reel]
    150   
    151   [movie reel]: movie.gif
...
be used as the caption.
    ...
    148   ![This is the caption](/url/of/image.png)
...
nonbreaking space after the image:
    ...
    149   ![This image won't be a figure](/url/of/image.png)\
...
Attributes can be set on links and images:
    ...
    150   An inline ![image](foo.jpg){#id .class width=30 height=20px}
    151   and a reference ![image][ref] with attributes.
    152   
    153   [ref]: foo.jpg "optional title" {#id .class key=val key2="val 2"}
...
\[above\]\[Extension: `native_divs`\]), HTML syntax can be used as part
of markdown to create native `Div` and `Span` elements in the pandoc AST
...
blocks (see \[Extension: `fenced_code_attributes`\]). As with fenced
...
*must* have attributes:
    ...
    152   ::: Warning ::::::
    153   This is a warning.
    154   
    155   ::: Danger
    156   This is a warning within a warning.
    157   :::
    158   ::::::::::::::::::
...
attributes:
    ...
    153   [This is *some text*]{.class key="val"}
...
Pandoc's Markdown allows footnotes, using the following syntax:
    ...
    154   Here is a footnote reference,[^1] and another.[^longnote]
    155   
    156   [^1]: Here is the footnote.
    157   
    158   [^longnote]: Here's one with multiple blocks.
    159   
    160       Subsequent paragraphs are indented to show that they
    161   belong to the previous footnote.
    162   
    163           { some.code }
    164   
    165       The whole paragraph can be indented, or just the first
    166       line.  In this way, multi-paragraph footnotes work like
    167       multi-paragraph list items.
    168   
    169   This paragraph won't be part of the note, because it
    170   isn't indented.
...
cannot contain multiple paragraphs). The syntax is as follows:
    ...
    155   Here is an inline note.^[Inlines notes are easier to write, since
    156   you don't have to pick an identifier and move down to type the
    157   note.]
...
is
    ...
    156   pandoc --filter pandoc-citeproc myinput.txt
...
YAML-encoded references, for example:
    ...
    157   ---
    158   references:
    159   - type: article-journal
    160     id: WatsonCrick1953
    161     author:
    162     - family: Watson
    163       given: J. D.
    164     - family: Crick
    ...
    169         - 4
    170         - 25
    171     title: 'Molecular structure of nucleic acids: a structure for deoxyribose
    172       nucleic acid'
    173     title-short: Molecular structure of nucleic acids
    174     container-title: Nature
    175     volume: 171
    176     issue: 4356
    177     page: 737-738
    178     DOI: 10.1038/171737a0
    179     URL: http://www.nature.com/nature/journal/v171/n4356/abs/171737a0.html
    180     language: en-GB
    181   ...
...
`csl` metadata field. By default, `pandoc-citeproc` will use the
\[Chicago Manual of Style\] author-date format. The CSL project provides
further information on \[finding and editing styles\].
...
(`:.#$%&-+?<>~/`). Here are some examples:
    ...
    158   Blah blah [see @doe99, pp. 33-35; also @smith04, chap. 1].
    159   
    160   Blah blah [@doe99, pp. 33-35, 38-39 and *passim*].
    161   
    162   Blah blah [@smith04; @doe99].
...
the text:
    ...
    159   Smith says blah [-@smith04].

You can also write an in-text citation, as follows:

    160   @smith04 says blah.
    161   
    162   @smith04 [p. 33] says blah.
...
document, for example:
    ...
    165   Title:   My title
    166   Author:  John Doe
    167   Date:    September 1, 2008
    168   Comment: This is a sample mmd title block, with
    169            a field spanning multiple lines.
...
Parses PHP Markdown Extra abbreviation keys, like
    ...
    166   *[HTML]: Hypertext Markup Language
...
[`link_attributes`](#extension-link_attributes) extension.
    ...
    167   This is a reference ![image][ref] with multimarkdown attributes.
    168   
    169   [ref]: http://path.to/image "Image title" width=20px height=30px
    170          id=myId class="myClass1 myClass2"
...
`markdown_phpextra` (PHP Markdown Extra) : `footnotes`, `pipe_tables`,
`raw_html`, `markdown_attribute`, `fenced_code_blocks`,
`definition_lists`, `intraword_underscores`, `header_attributes`,
...
`raw_html`, `fenced_code_blocks`, `gfm_auto_identifiers`,
...
`tex_math_double_backslash`, `intraword_underscores`, `mmd_title_block`,
`footnotes`, `definition_lists`, `all_symbols_escapable`,
...
Here's the Markdown source for a simple slide show, `habits.txt`:
    ...
    191   ------------------
    192   
    193   ![picture of spaghetti](images/spaghetti.jpg)
    194   
    195   ## Going to sleep
    196   
    197   - Get in bed
...
To produce an HTML/JavaScript slide show, simply type
    ...
    169   pandoc -t FORMAT -s habits.txt -o habits.html
...
To produce a PDF slide show using beamer, type
    ...
    170   pandoc -t beamer habits.txt -o habits.pdf
...
To produce a Powerpoint slide show, type
    ...
    171   pandoc habits.txt -o habits.pptx
...
three dots, separated by spaces:
    ...
    175   # Slide with a pause
    176   
    177   content before the pause
    178   
    179   . . .
    180   
    181   content after the pause
...
`fonttheme`, `innertheme`, and `outertheme`, using the `-V` option:
    ...
    177   pandoc -t beamer habits.txt -V theme:Warsaw -o habits.pdf
...
bibliographies:
    ...
    178   # References {.allowframebreaks}
...
option `parallaxBackgroundImage` either in the YAML metadata block or as
a command-line variable. You can also set `parallaxBackgroundHorizontal`
...
`data-background-size`, `data-background-repeat`,
...
example:
    ...
    185     text: My Book
    186   - type: subtitle
    187     text: An investigation of metadata
    188   creator:
    189   - role: author
    190     text: John Smith
    191   - role: editor
    192     text: Sarah Jones
    193   identifier:
    194   - scheme: DOI
    195     text: doi:10.234234.234/33
    196   publisher:  My Press
    197   rights: © 2007 John Smith, CC BY-NC
    198   ibooks:
    199     version: 1.3.4
    200   ...
...
`identifier` ~ Either a string value or an object with fields `text` and
...
`type`, or a list of such objects. Valid values for `type` are `main`,
...
`role` are \[MARC relators\], but pandoc will attempt to translate the
...
`lang` (or legacy: `language`) ~ A string value in \[BCP 47\] format.
...
`page-progression-direction` attribute for the \[`spine` element\].

`ibooks` ~ iBooks-specific metadata, with the following fields:

    183   - `version`: (string)
    184   - `specified-fonts`: `true`|`false` (default `false`)
    185   - `ipad-orientation-lock`: `portrait-only`|`landscape-only`
    186   - `iphone-orientation-lock`: `portrait-only`|`landscape-only`
    187   - `binding`: `true`|`false` (default `true`)
    188   - `scroll-axis`: `vertical`|`horizontal`|`default`
...
tag with the `src` attribute. For example:
    ...
    184   <audio controls="1">
    185     <source src="http://example.com/music/toccata.mp3"
    186             data-external="1" type="audio/mpeg">
    187     </source>
    188   </audio>
...
Without the `+styles` extension:
    ...
    185   $ pandoc test/docx/custom-style-reference.docx -f docx -t markdown
    186   This is some text.
    187   
    188   This is text with an *emphasized* text style. And this is text with a
    189   **strengthened** text style.
    190   
    191   > Here is a styled paragraph that inherits from Block Text.

And with the extension:

    186   $ pandoc test/docx/custom-style-reference.docx -f docx+styles -t markdown
    187   
    188   ::: {custom-style="FirstParagraph"}
    189   This is some text.
    190   :::
    191   
    192   ::: {custom-style="BodyText"}
    193   This is text with an [emphasized]{custom-style="Emphatic"} text style.
    194   And this is text with a [strengthened]{custom-style="Strengthened"}
    195   text style.
    196   :::
    197   
    198   ::: {custom-style="MyBlockStyle"}
    199   > Here is a styled paragraph that inherits from Block Text.
    200   :::
...
you can modify according to your needs, do
    ...
    190   pandoc --print-default-data-file sample.lua
