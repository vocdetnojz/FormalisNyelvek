...
type CountStateList = ([String], Int)
type CountStateString = (String, Int)
...
callNumberLine :: [String] -> Int -> ([String], Int)
callNumberLine xs a = runState (mapM numberLine xs) a

splitModJoin :: String -> Int -> (String, Int)
splitModJoin xs a = (join (fst (callNumberLine (splitOn "\n" xs) a)), (snd (callNumberLine (splitOn "\n" xs) a)))


evalOnBlock :: Block -> State Int Block
evalOnBlock (CodeBlock attr xs) = do
    i <- newId
--    cb <- CodeBlock attr (snd (splitModJoin xs i))
--    pure $ CodeBlock attr (fst (splitModJoin xs newId))
    pure $ CodeBlock attr (fst (splitModJoin xs i))
evalOnBlock x = pure x
